...
local calc_ratio = function()
	...
	if c.ref_editor.columns > 0 then
		col_ratio = vim.o.columns / c.ref_editor.columns
	end

	if c.ref_editor.lines > 0 then
		line_ratio = vim.o.lines / c.ref_editor.lines
	end
	return col_ratio, line_ratio
end

local adjust_win = function(col_ratio, line_ratio, c_win)
	local adjusted_cols = math.ceil(c_win.columns * col_ratio)
	local adjusted_lines = math.ceil(c_win.lines * line_ratio)
